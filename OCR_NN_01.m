...
function dist = NDDist(v1, v2)
d = (v1-v2).^2;
dist = sum(d)^0.5;
end
function prediction = NNTesting(testImage, modelNN)
%Find distance from test image to each of the "zeros" and "ones"
nearest = [modelNN.labels(1) NDDist(testImage, modelNN.neighbours(1,:))];
for i = 1:size(modelNN.labels)
    nd = NDDist(testImage, modelNN.neighbours(i,:));
    if(nd < nearest(2))
        nearest=[modelNN.labels(i) nd];
    end
end
